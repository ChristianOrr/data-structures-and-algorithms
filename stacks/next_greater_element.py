...
the input array that's greater than inputArray[i].
...
def nextGreaterElement(array):
    greater_array = [-1 for i in range(len(array))]
    stack = []
    for i in reversed(list(range(len(array) * 2))):
        index = i % len(array)
        while len(stack) != 0:
            if stack[-1] > array[index]:
                greater_array[index] = stack[-1]
                break
            stack.pop()
...
class TestProgram(unittest.TestCase):
    ...
    def test_2(self):
        input = [0, 0, 5, 0, 0, 3, 0]
        expected = [5, 5, -1, 3, 3, 5, 5]
        actual = nextGreaterElement(input)
        self.assertEqual(actual, expected)

    def test_3(self):
        input = [1, 2, 3, 4, 1, 2, 3, 4, -8, -7, 6, 2, 17, 2, -8, 9, 0, 2]
        expected = [2, 3, 4, 6, 2, 3, 4, 6, -7, 6, 17, 17, -1, 9, 9, 17, 2, 3]
        actual = nextGreaterElement(input)
        self.assertEqual(actual, expected)
